...
public class DataPersistence {
    ...
    public void createTableSchemaWithDomains(String table, model.Attribut[] attributs, String databasePath) {
        ...
        try {
            ...
            System.out.println(
                    "[Persistence] Schéma créé pour " + table + " avec domaines complets dans " + databasePath);
        } catch (IOException e) {
            System.err.println("Erreur création schéma: " + e.getMessage());
        }
    }
    ...
    public Relation loadRelationFromFile(String tableName, String databasePath) {
        try {
            ...
            String[] columnDomains = schemaLines.size() > 1 ? schemaLines.get(1).split("\\|")
            ...
            for (int i = 0; i < columnNames.length; i++) {
                ...
                String domainSpec = i < columnDomains.length && columnDomains[i] != null ? columnDomains[i].trim()
                        : "String||";
            ...
            if (dataPath != null) {
                ...
                System.out.println(
                        "[Persistence] Relation " + tableName + " chargée avec " + relation.getNbTuples() + " tuples");
            }
    ...
    public void saveDomaine(String name, Domaine domaine, String databasePath) {
        String filename = databasePath + java.io.File.separator + name + "_domaine.txt";
        try {
            Files.createDirectories(Paths.get(databasePath));

            String content = domaine.serialize();
            Files.write(Paths.get(filename), content.getBytes());
            System.out.println("[Persistence] Domaine sauvegardé: " + name + " dans " + databasePath);
        } catch (IOException e) {
            System.err.println("Erreur sauvegarde domaine: " + e.getMessage());
        }
    }

    public void deleteDomaine(String name, String databasePath) {
        String filename = name + "_domaine.txt";
        Path domainPath = findFileInDatabaseIgnoreCase(filename, databasePath);
        if (domainPath != null) {
            try {
                Files.delete(domainPath);
                System.out.println("[Persistence] Domaine supprimé: " + name);
            } catch (IOException e) {
                System.err.println("Erreur suppression domaine: " + e.getMessage());
            }
        }
    }

    public Map<String, Domaine> loadAllDomaines(String databasePath) {
        Map<String, Domaine> domaines = new HashMap<>();
        try {
            Path dir = Paths.get(databasePath);
            if (!Files.exists(dir)) {
                return domaines;
            ...
            try (DirectoryStream<Path> stream = Files.newDirectoryStream(dir)) {
                for (Path path : stream) {
                    String fileName = path.getFileName().toString();
                    if (Files.isRegularFile(path) && fileName.endsWith("_domaine.txt")) {
                        String domaineName = fileName.substring(0, fileName.length() - "_domaine.txt".length());
                        try {
                            String content = new String(Files.readAllBytes(path));
                            Domaine domaine = Domaine.deserialize(domaineName, content);
                            domaines.put(domaineName, domaine);
                            System.out.println("[Persistence] Domaine chargé: " + domaineName);
                        } catch (Exception e) {
                            System.err.println("Erreur chargement domaine " + domaineName + ": " + e.getMessage());
                        }
                    }
                }
            }
        } catch (IOException e) {
            System.err.println("Erreur lecture répertoire domaines " + databasePath + ": " + e.getMessage());
        }
        return domaines;
    }
}
